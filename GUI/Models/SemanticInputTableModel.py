...
IN_TYPE = SemanticDataType.IN

class SemanticInputTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=None):
        if role == Qt.DisplayRole or role == Qt.EditRole:
            if index.column() == 0:
                semantic_object = SemanticData.get_semantic_type_object_at(index.row(), IN_TYPE)
                return QVariant(semantic_object)
            elif index.column() == 1:
                interaction = SemanticData.get_semantic_type_interaction_at(index.row(), IN_TYPE)
                return QVariant(interaction)
        else:
            return QVariant()

    def rowCount(self, parent=QModelIndex()):
        return SemanticData.get_semantic_type_len_at_selected(IN_TYPE)
    ...
    def setData(self, index, value, role=Qt.DisplayRole):
        if index.isValid() and role == Qt.EditRole and value:
            ...
            SemanticData.update_semantic_type_at(index.row(), IN_TYPE, node_type, value)
            self.dataChanged.emit(index, index, [])
            return True
        return False

    def insert_new_input(self):
        self.insertRow(SemanticData.get_semantic_type_len_at_selected(IN_TYPE))
        SemanticData.add_new_semantic_type(IN_TYPE)

    def clear_all_inputs(self):
        # remove rows workaround
        a = 0
        if -1 != SemanticData.get_semantic_type_len_at_selected(IN_TYPE) - 1:
            a = SemanticData.get_semantic_type_len_at_selected(IN_TYPE) - 1

        self.removeRows(0, a)

    def request_input_for_selected(self):
        self.insertRows(0, SemanticData.get_semantic_type_len_at_selected(IN_TYPE) - 1)
        self.dataChanged.emit(QModelIndex(), QModelIndex(), [])

    def remove_input_at(self, index):
        if index > -1:
            self.removeRow(index)
            SemanticData.remove_selected_semantic_type_at(index, IN_TYPE)
